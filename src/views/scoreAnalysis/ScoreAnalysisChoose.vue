<template>
  <div style="width: 69%; margin: 0 auto">
      ...
      <el-table :data="tableData" :border=true stripe style="width:100%; margin: auto" highlight-current-row>
        ...
        <el-table-column align="center" min-width="130%">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="warning" @click.native="handleKnowledgeStatistics(scope.$index,scope.row)"
    ...
    <div ref="examGroupView" style="display: none">
      <el-button type="primary" @click.native="handleMore2" icon="el-icon-plus" style="float: right" v-text="more">查看更多
      </el-button>
      <el-table :data="tableDataExamGroup" :border=true stripe style="width:100%; margin: auto">
        <el-table-column align="left" prop="name" label="考试名称" :xs="4" :sm="4" :md="4" :lg="3"></el-table-column>
        <el-table-column align="left" prop="createUserName" label="创建人" :xs="4" :sm="5" :md="4"
                         :lg="3"></el-table-column>
        <el-table-column align="left" prop="createTime" label="创建时间" :formatter="dateFormat" :xs="4" :sm="5" :md="4"
                         :lg="3"></el-table-column>
        <el-table-column align="center" label="动作" :xs="10" :sm="10" :md="4" :lg="3" min-width="200%">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="danger" @click.native="handlePrint3(scope.$index,scope.row)">贡献率
            </el-button>
            <el-button size="mini" type="warning" @click.native="handlePrint4(scope.$index,scope.row)">班级排名</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="pagination-container">
  ...
  export default {
    ...
    methods: {
      ...
      handlePrint(index, row){
        this.$router.push({path: '/exam/examGroupToPrint', query: {multiTestID: row.id, schoolID: row.schoolID}})
      },
      ...
      handleKnowledgeStatistics(index,row){
        this.$router.push({path: "/scoreAnalysis/knowledgeStatistics/", query: {testID: row.id, name: row.name}})
      }
    }
  }
</script>
