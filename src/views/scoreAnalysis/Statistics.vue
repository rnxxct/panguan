...
<!--
          <el-button type="primary" @click="handleCheck">查看图片</el-button>
-->
          ...
          <!-- <div style="border: 2px solid black; width: 100%;height: 100%;padding: 50px 50px;">
             <span>正答人数:{{}}:</span>
             <el-input
      ...
      <!--
          <iframe src="" name="show" style="width: 0px;height: 0px;"></iframe>
      -->
    </div>
...
<script>
  import {getTree, getData, exportEXCEL, getImg} from  '@/api/scoreAnalysis/statistics'
        ...
        if (bar) {
          ...
          mybar.setOption({
            title: {
              ...
              subtext: '难度:' + this.difficulty + '区分度:' + this.difference,
            },
            tooltip: {
              trigger: 'axis',
          ...
          mybar.on('legendselectchanged', function (obj) {
            ...
            for (var s1 in  obj.selected) {
              if (obj.name == s1) {
                that.difference = that.studentsQuestionResultsOfClasses[j].difference
            ...
            option.title[0].subtext = '难度:' + that.difficulty + '区分度:' + that.difference
        ...
        exportEXCEL({
          ...
          import('@/vendor/Export2Excel').then(excel => {
            ...
            excel.export_json_to_excel({
              header: tHeader,
              data,
              filename: this.testName + "_" + this.classes[this.currentClassIndex].text + "班 单题",
