...
<script>
    ...
    import {getQuestionAndKnowledge,updateBreakdown} from '@/api/exam/breakdown'
        ...
        methods: {
            initTree() {
                ...
                getQuestionAndKnowledge(this.listQuery).then(response => {
                    ...
                    this.editParams.examName = this.$route.query.name
                    console.log(this.dataSource)
                    for (let i = 0; i < this.dataSource.length; i++) {
                        if (this.dataSource[i].type == '主观题' && this.dataSource[i].children[0].knowledges.length > 0) {
                            this.dataSource[i].knowledges = this.dataSource[i].children[0].knowledges;
                        }
                    }
                })
            },
            ...
            toEdit() {
                ...
                updateBreakdown(this.editParams).then(response => {
                    this.$message({
                        message: '新增细目表成功',
                        type: "success",
                        duration: 600
            ...
            handleExport() {
                var array = new Array();
                for (let i = 0; i < this.dataSource.length; i++) {
                    if (this.dataSource[i].type == '选择题') {
                        for (let j = 0; j < this.dataSource[i].children.length; j++) {
                            if (this.dataSource[i].children[j].knowledges.length > 0) {
                                for (let k = 0; k < this.dataSource[i].children[j].knowledges.length; k++) {
                                    let str = this.dataSource[i].children[j].knowledges[k]
                                    let data= this.options5.filter(function(item){
                                        return item.value == str;
                                    })
                                    console.log(data[0])
                                    if (undefined !== this.dataSource[i].children[j]['knowledge']) {
                                        this.dataSource[i].children[j]['knowledge'] = this.dataSource[i].children[j]['knowledge'] + data[0].label+'、';
                                    } else {
                                        this.dataSource[i].children[j]['knowledge'] = data[0].label+'、';
                                    }

                                }
                                array.push(this.dataSource[i].children[j]);
                            }
                            array.push(this.dataSource[i].children[j]);
                        }
                    } else {
                        if (this.dataSource[i].knowledges.length > 0) {
                            for (let k = 0; k < this.dataSource[i].knowledges.length; k++) {
                                let str = this.dataSource[i].knowledges[k]
                                let data= this.options5.filter(function(item){
                                    return item.value == str;
                                })
                                if (undefined !== this.dataSource[i]['knowledge']) {
                                    this.dataSource[i]['knowledge'] = this.dataSource[i]['knowledge'] + data[0].label+'、';
                                } else {
                                    this.dataSource[i]['knowledge'] = data[0].label+'、';
                                }
                            }
                ...
                import('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['题目', '类型', '知识点']
                    const filterVal = ['lable', 'type', 'knowledge']
                    const data = this.formatJson(filterVal, array)
                    excel.export_json_to_excel({
                        header: tHeader,
                        data,
                        filename: this.$route.query.name + "_" + '细目表',
