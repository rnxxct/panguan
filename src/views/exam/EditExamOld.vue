<template>
  <div style="width: 69%;margin: 0 auto">
    ...
    <el-table
      ...
      <el-table-column label="满分" align="left">
        <template slot-scope="scope">
        <span v-if="spaceIconShow(1)" v-for="(space, levelIndex) in scope.row._level" class="ms-tree-space"
              :key="levelIndex"></span>
          <span v-if="toggleIconShow(1,scope.row)" @click="toggle(scope.$index)">
          ...
          <el-input v-if="scope.row.type!=='选择题'" style="width: 50px; display: inline-block" v-model="scope.row.fullScore" type="text" size="mini">
            {{scope.row.fullScore}}
          </el-input><br/>
          <span v-if="!scope.row._expanded && scope.row.type==='多项选择题'">漏选分值:</span>
          <el-input style="width: 50px; display: inline-block" type="text" v-if="!scope.row._expanded && scope.row.type==='多项选择题'"
...
<script>
  ...
  import {getTree, updateExam} from '@/api/exam/updateExam'
