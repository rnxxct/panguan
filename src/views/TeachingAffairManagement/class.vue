      ...
      <el-table-column align="left" min-width="50%" prop="action" label="动作">
        <template slot-scope="scope">
          <!--          <el-button
                      size="mini"
                      type="primary" plain
                      @click="handleSearch(scope.$index, scope.row)">班级管理-->
          <!--
                    </el-button>
          -->
          <el-button size="mini" type="primary" plain>
            <router-link :to="'/teachingAffair/classOfSchool/'+scope.row.id" class="link-type">
              <span>班级管理</span>
            </router-link>
          </el-button>
        </template>
...
<script>
  import {getList} from '@/api/TeachingAffair/class'
