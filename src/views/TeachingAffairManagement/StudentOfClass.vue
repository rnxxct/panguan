<template>
  <div class="inner_element" style="width: 69%;margin: 0 auto;">
    ...
    <el-button type="success" @click.native="handUpload" icon="el-icon-plus" style="float: right;margin-right: 20px">
      导入花名册
    </el-button>
    <el-button type="primary" @click.native="handleAdd" icon="el-icon-plus" style="float: right;margin-right: 20px">新增学生
    </el-button>
        ...
        <template slot-scope="scope">
          ...
          <!--
                    <el-button
                      size="mini"
                      type="primary" plain
                      @click="handleClass(scope.$index, scope.row)">转班
                    </el-button>
          -->
        </template>
      </el-table-column>
...
<script>
  import {getList as getClasses} from '@/api/TeachingAffair/classOfSchool'
  import {
    getList,
    addStudent,
    deleteStudent,
  ...
  export default {
    ...
    created(){
      ...
      this.addForm.classID = this.$route.params.classID
      /*
