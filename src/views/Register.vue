        ...
        <div class="login">
          ...
          <form method="get" style="margin-bottom: 16px;" ref="ruleForm2" :rules="rules2">
            <div class="input_control">
              <input type="text" v-model="ruleForm2.account" class="from_input" placeholder="请输入用户名"/>
            </div>
            <div class="input_control">
              <input type="text" v-model="ruleForm2.phone" class="from_input" placeholder="请输入手机号"/>
            </div>
            <div class="input_control">
              <input type="password" v-model="ruleForm2.checkPass" class="from_input" placeholder="请输入密码"/>
            </div>
            <div class="input_control">
              <input type="password" v-model="ruleForm2.checkPass2" class="from_input" placeholder="确认密码"/>
            </div>
            <div class="input_control">
              <input id="btn1" type="button" @click="handleSubmit2" value="注 册"/>
            </div>
          </form>
        </div>
      </div>
  ...
  export default {
    ...
    created(){
      ...
      if (mb !== 'Chrome' && mb !== 'Chromium') {
        alert("您当前使用的不是chrome浏览器,出现问题我们不负责任!")
      }
    },
    methods: {
      myBrowser(){
      ...
      handleSubmit2(ev) {
        ...
        if (this.ruleForm2.checkPass !== this.ruleForm2.checkPass2) {
          this.$message({
            message: '两次密码不相同!',
            type: "warning",
            duration: 600
        ...
        if (this.ruleForm2.account !== '' && this.ruleForm2.checkPass !== '' && this.checkPass==this.checkPass2){
          this.requestRegister()
        }
      },
      requestRegister(){
        register(this.ruleForm2.account, this.ruleForm2.checkPass,this.ruleForm2.phone).then(data => {
            if (data.code == 200) {
              this.$message({
                message: '注册成功',
                type: "success",
